...
- `config_schema.py` imports from `plugins.memory.config_schema` on purpose: Hermes core loads
  that file directly and its dashboard expects core's `ProviderConfigSchema` type, so it must
  not be vendored.
